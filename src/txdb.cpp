...
bool CCoinsViewDB::GetRichPriorities(addressPrioritySet &priorities) const {

    //there are no const_iterators for leveldb so we use a cast since we only need read operations
        ...
        { 
            ...
            if (key.first == DB_ADDRESSINDEX) 
            {
                ...
                if(value.first >= RICH_AMOUNT) {
                    if(!priorities.insert(CAddressPriority(key.second, value.second)).second)
                        return false;
                }
                pcursor->Next();
            } else {

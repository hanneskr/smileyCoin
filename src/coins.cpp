...
bool CCoinsView::GetRichPriorities(addressPrioritySet &priorities) const { return false; }
...
bool CCoinsViewBacked::GetRichPriorities(addressPrioritySet &priorities) const { return base->GetRichPriorities(priorities); }
...
bool CCoinsViewCache::GetRichPriorities(addressPrioritySet &priorities) const {
    for (std::map<CScript, std::pair<int64_t,int> >::const_iterator it = cacheAddressIndex.begin(); it != cacheAddressIndex.end(); it++) {
        if(it->second.first >= RICH_AMOUNT) {
            if(!priorities.insert(CAddressPriority(it->first, it->second.second)).second)
                return false;
        }
    }
    return true;
